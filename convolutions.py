# -*- coding: utf-8 -*-
"""
Created on Sat Feb  9 21:46:43 2019
...
def convolve(image, K):
    # grab the spatial dimensions fo the image and kernel
    (iH, iW) = image.shape[:2]
    (kH, kW) = K.shape[:2]
    # allocate memory for the output image, taking care to "pad" the borders
    # of the input image so the spatial size (i.e., width and heigth) are not reduced
    pad = (kW -1) // 2
    ...
    for y in np.arange(pad, iH + pad):
        for x in np.arange(pad, iW + pad):
            # exgtract the ROI(region of interest) of the image
            # by extracting the *center* region of the current (x, y)-coordinates dimensions
...
ap.add_argument("-i", "--image", required=False, default="datasets/jemma.png", help="path to the input image")
...
# construct the Laplacian kenel used to detect edge-like regions of an image
